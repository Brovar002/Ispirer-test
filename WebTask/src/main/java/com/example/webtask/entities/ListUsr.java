...
import com.example.webtask.entities.event.ChangedListenerImpl;
import com.example.webtask.entities.event.ListenerManager;
import com.example.webtask.entities.event.ListenerType;
...
public class ListUsr<E> {
    ...
    public String add(E element) {
        add(size, element);
        if (listener != null) {
            return events.notify(ListenerType.CHANGED, element) + "<button type=\"button\" onclick=\"location.href='/list'\"><-</button>";
        } else {
            return "<button type=\"button\" onclick=\"location.href='/list'\"><-</button>";
        }
    }
